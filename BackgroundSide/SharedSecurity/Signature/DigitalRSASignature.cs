...
{
    public class DigitalRSASignature
    {
        RSAParameters publicKey, privateKey;
        public void AssignNewKey(string identification)
        {
            using (var rsa = new RSACryptoServiceProvider(2048))
            {
                ...
                //SAVE BOTH KEYS IN ENTITYACCOUNT. SHARE PUBLIC KEY WITH CLIENT
                var strPublicKeyExported = ExportPublicKeyToPEMFormat(rsa);
                var strPrivateKeyExported = ExportPrivateKey(rsa);

                SharedBusiness.Security.BlSecurity blSecurity = new SharedBusiness.Security.BlSecurity();

                blSecurity.SetKeyPair(identification, strPrivateKeyExported, strPublicKeyExported);
        ...
        public void ImportPrivateKey(string PemPrivateKey)
        {
            ...
            var privateKeyBits = System.Convert.FromBase64String(PemPrivateKey);

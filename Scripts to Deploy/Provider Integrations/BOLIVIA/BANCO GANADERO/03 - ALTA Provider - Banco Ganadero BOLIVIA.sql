...
DECLARE @PAIS				VARCHAR(MAX)	= 'BOLIVIA (PLURINATIONAL STATE OF)';			-- Ej. EJEMPLO ARGENTINA
DECLARE @CODE				VARCHAR(7)		= 'BGBOL';			-- Ej. ABC
DECLARE @NAME				VARCHAR(60)		= 'BANCO GANADERO'; -- Ej. Banco ABC
DECLARE @LONG_DESCRIPTION	VARCHAR(60)		= 'Banco Ganadero (Bolivia)'; -- Ej  Banco ABC de Formosa

DECLARE @JUST_TESTING BIT = 0
...
BEGIN TRY
	...
	IF (@JUST_TESTING = 1)
		BEGIN
			PRINT 'MODO Testing... Realizando Rollback';
			ROLLBACK TRAN;
		END
	ELSE

...
DECLARE @idProvider INT = (SELECT DISTINCT [idProvider] FROM [LP_Configuration].[Provider] WHERE Code = 'STRPER' AND [Active] = 1) 
DECLARE @idCountry INT = (SELECT DISTINCT [idCountry] FROM [LP_Location].[Country] WHERE Name = 'PERU' AND [Active] = 1) 

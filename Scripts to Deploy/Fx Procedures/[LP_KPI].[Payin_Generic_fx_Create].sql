CREATE OR ALTER PROCEDURE [LP_KPI].[Payin_Generic_fx_Create]
																	(
...
BEGIN
				...
				BEGIN TRANSACTION
					...
					BEGIN
						...
						BEGIN
							...
							SET @fxValue = ( SELECT [Value] FROM [LP_Configuration].[CurrencyExchange] WHERE [idCurrencyExchange] = @idCurrencyExchange ) * ( 1 - @Spread_fxValue_Sell / 100.0 )
			...
			-----------------------------------------------------------------------------------------------------------------------------
							...
							(
								...
								, @TransactionDate
								, @Value
								, @fxClientTaxWithHoldings
								, @fxClientTaxWithHoldingsARBA
								, @fxClientNet
								, @ClientCommission
								, @fxClientVAT				

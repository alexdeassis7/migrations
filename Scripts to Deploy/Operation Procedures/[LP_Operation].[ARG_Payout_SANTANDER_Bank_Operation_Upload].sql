...
CREATE OR ALTER PROCEDURE [LP_Operation].[ARG_Payout_SANTANDER_Bank_Operation_Upload]
                                      (
										@JSON					NVARCHAR(MAX)
...
BEGIN TRY
    ...
    BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          ( SELECT TOP 1 [CE].[idCurrencyExchange] 
            ...
            WITH ( INDEX=IX_LP_Configuration_CurrencyExchange_Action_CurrencyTo_Active ) 
            WHERE [CE].[Active] = 1 
            AND [CE].[ActionType] = 'A' 
        ...
        BEGIN
          ...
          BEGIN
            SELECT 
              @Value_Spot = [Value]
            FROM 
              [LP_Configuration].[CurrencyExchange]
            WHERE 
      ...
      BEGIN
        EXEC [LP_Operation].[Return_Transaction_Upload] @Ticket,'ARG','SANTAR','RETURNED'
        INSERT INTO @TxsDetail
          SELECT 
            [Ticket] = [T].[Ticket] ,
            [TransactionDate] = CONVERT(DATETIME2, [T2].[TransactionDate], 1) ,

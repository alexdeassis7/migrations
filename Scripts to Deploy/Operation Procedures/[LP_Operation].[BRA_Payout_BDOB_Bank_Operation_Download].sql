...
CREATE OR ALTER PROCEDURE [LP_Operation].[BRA_Payout_BDOB_Bank_Operation_Download]
																			(
...
BEGIN
			...
			BEGIN TRY
					...
					SET @LPBankBranch = '03347'

					DECLARE @LPBankAccountNumber AS VARCHAR(12)
					SET @LPBankAccountNumber = '000000006436'

					DECLARE @LPBankAccountVerifier AS CHAR(1)
					SET @LPBankAccountVerifier = 'X'

					DECLARE @LPCodigoConvenio AS VARCHAR(20)
					SET @LPCodigoConvenio = '0002625780126       '
					...
					SELECT 
						...
						IIF([BC].[Code] = @BankCode,RIGHT([TRD].[BankBranch], 1) ,' '),
					...
					BEGIN
						...
						SELECT 
							...
							REPLICATE('0', 8) AS [VencimientoPago],
			...
			/* HEADER BLOCK: INI */
					...
					DECLARE @HDigVerifAg AS VARCHAR(1) = '2'
					...
					DECLARE @HNomBanco AS VARCHAR(30) = LEFT('BANCO DO BRASIL S.A.' + SPACE(30), 30)
					...
					DECLARE @HCnab4 AS VARCHAR(15) = REPLICATE('0', 15)

...
ALTER   PROCEDURE [LP_Operation].[BRA_Payout_SAFRA_Bank_Operation_Upload](
					@JSON					NVARCHAR(MAX), 
...
BEGIN
  BEGIN TRY
    ...
    SET @idProvider = ( SELECT [idProvider] FROM [LP_Configuration].[Provider] WHERE [Code] = 'SAFRA' AND [idCountry] = @idCountry AND [Active] = 1 )
  ...
  BEGIN
    ...
    WHERE [idTransaction] IN
                (
                  SELECT [idTransaction] FROM [LP_Operation].[Ticket] WHERE [TicketAlternative] = @Ticket
                )
  END
  ...
  BEGIN
      ...
      WHERE [T].[TicketAlternative] = @Ticket 
  ...
  BEGIN 
    ...
    ELSE 
    BEGIN
	IF (@StatusTransaction = @idStatus AND @TxIdStatus = @idStatusRejected)
	BEGIN
		SET @TicketLP = (SELECT Ticket FROM [LP_Operation].[Ticket] WHERE TicketAlternative = @Ticket)
		EXEC [LP_Operation].[Return_Transaction_Upload] @TicketLP,'BRA','SAFRA','RETURNED'
		INSERT INTO @TxsDetail
          SELECT [Ticket] = [T].[Ticket] ,
              [TransactionDate] = CONVERT(DATETIME2, [T2].[TransactionDate], 1) ,
              [Amount] = [T2].[GrossValueLP] ,

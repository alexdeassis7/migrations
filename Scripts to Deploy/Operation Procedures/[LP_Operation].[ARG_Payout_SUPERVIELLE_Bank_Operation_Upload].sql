...
ALTER PROCEDURE [LP_Operation].[ARG_Payout_SUPERVIELLE_Bank_Operation_Upload]
                                      (
										@JSON					NVARCHAR(MAX)
...
BEGIN TRY
	...
	BEGIN
      ...
      WHERE
        [T].[TicketAlternative] = RIGHT(@Ticket, 10)
      ...
      BEGIN
        ...
        BEGIN
          ...
          BEGIN
            ...
            BEGIN
              SET @BeforeValueBalance = ROUND(((@GrossAmount / @ValueFX) * @SignMultiply) - @CommissionWithVAT, 2) - @LocalTax
  ...
  BEGIN CATCH
    ...
    DECLARE @ErrorMessage NVARCHAR(4000) = ERROR_MESSAGE() + ',linea: ' + cast(ERROR_line() as varchar(10))

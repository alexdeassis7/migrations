...
CREATE OR ALTER PROCEDURE [LP_Operation].[BRA_Payout_PLURAL_Bank_Operation_Upload]
																			(
																				@JSON					NVARCHAR(MAX)
																				, @CurrencyExchangeClose	BIGINT
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE
			...
			SET @idProvider = ( SELECT [idProvider] FROM [LP_Configuration].[Provider] WHERE [Code] = 'PLURAL' AND [idCountry] = @idCountry AND [Active] = 1 )
	 ...
	 DECLARE @ticketTable TABLE (
	  ...
	  DECLARE @TxsDetail TABLE(
		...
		WITH (   
				...
				reject  varchar(200) '$.rejectDetail'
			);

	BEGIN TRAN

	DECLARE payout_cursor CURSOR FORWARD_ONLY FOR
	SELECT ticket,detail,rejectDetail FROM  @ticketTable

	OPEN payout_cursor;
	FETCH NEXT FROM payout_cursor INTO @ticket, @DetailStatus, @RejectionDetail
	...
	BEGIN
			...
			BEGIN
				SELECT @DetailStatus = [Code]
				FROM [LP_Configuration].[InternalStatus]
				WHERE
					[Active] = 1
					AND [idProvider] = @idProvider
					AND [idCountry] = @idCountry
					AND [Name] = LEFT(@RejectionDetail, 60)

				IF @DetailStatus IS NULL
				BEGIN
					SET @DetailStatus = 'REJECTED'
				END
			END
			...
			BEGIN
				...
				WHERE [idTransaction] IN
										(
											SELECT [idTransaction] FROM [LP_Operation].[Ticket] WHERE [Ticket] = @Ticket
										)
			END
			...
			BEGIN
				...
				BEGIN
					...
					SELECT
						...
						, [InternalStatus]				= CASE 
															WHEN @isError = 1 AND @isFinalStatus = 1 THEN 'ERROR'
															WHEN @isError = 1 AND @isFinalStatus = 0 THEN 'PENDING'
															WHEN @isError = 0 AND @isFinalStatus = 1 THEN 'OK'
															WHEN @isError = 0 AND @isFinalStatus = 0 THEN 'PENDING'
														  END

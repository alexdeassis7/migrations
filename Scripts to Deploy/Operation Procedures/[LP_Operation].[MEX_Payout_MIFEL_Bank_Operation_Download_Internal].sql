...
CREATE OR ALTER PROCEDURE [LP_Operation].[MEX_Payout_MIFEL_Bank_Operation_Download_Internal]
																					(
...
BEGIN
	...
	BEGIN TRY
			...
			SELECT
				[RegistryType]					= '10'
				, [BeneficiaryAccount]			= SUBSTRING([TRD].[RecipientAccountNumber], 7, 11)
			...
			WHERE [T].[idStatus] = [LP_Operation].[fnGetIdStatusByCode]('Received')
				AND [TD].[NetAmount] > 0
			...
			SELECT
				...
				[AccountNumber]					= SUBSTRING([TRD].[RecipientAccountNumber], 7, 11)
				, [MaxAmount]					= '20000000'				
			...
			SET [LineComplete] = CAST([idx] AS VARCHAR) + @FieldSeparator + '10' + @FieldSeparator + '' + @FieldSeparator + '' + @FieldSeparator + [AccountNumber] + @FieldSeparator + [MaxAmount] + @FieldSeparator + [BeneficiaryName] + @FieldSeparator

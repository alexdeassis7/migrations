...
CREATE OR ALTER PROCEDURE [LP_Operation].[CHL_Payout_ITAU_Bank_Operation_Upload]
                                                                          (
                                                                            @JSON                     NVARCHAR(MAX)
...
BEGIN
    ...
    DECLARE
		...
		SET @idCountry = ( SELECT [idCountry] FROM [LP_Location].[Country] WHERE [ISO3166_1_ALFA003] = 'CHL' AND [Active] = 1 )
	...
	BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          UPDATE  [LP_Operation].[TransactionInternalStatus]
          SET   
            [idInternalStatus] = [LP_Operation].[fnGetIdInternalStatusByCodeCatalogCountryProvider](44, @idProvider, @DetailStatus, 'SCM')
          WHERE 
            [idTransaction] = @idTransaction

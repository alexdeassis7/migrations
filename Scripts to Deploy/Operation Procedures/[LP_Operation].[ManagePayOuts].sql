CREATE OR ALTER  PROCEDURE [LP_Operation].[ManagePayOuts]
													(
...
AS
	BEGIN
		...
		SET @idProvider = (SELECT idProvider FROM [LP_Configuration].[Provider] WHERE Code = JSON_VALUE(@JSON, '$.provider') AND idCountry = @idCountry)

    ...
    BEGIN
      ...
      SET @RESP =
                    ...
                    SELECT 
                      ...
                      [CollectionDate]      = IIF
                                      (
                                        ...
                                        NULL,                               
										IIF([TCAPS].[RealProviderCashingDate] IS NOT NULL,
                      ...
                      [InternalClient_id]     = IIF([TO].[Code] = 'PO', ISNULL([TRD].[InternalDescription], [BCT].[Invoice]), IIF([TO].[CODE] = 'PI', [TPID].[MerchantId],'')),
                      [Status]          = [S].[Code],
                      [DetailStatus]        = IIF([TO].[Code]='LP-Action', REPLACE([TD2].[Description], '&', ''), ISNULL([LPIE].[Name], [IS].[Description])),
                      [Pay]           = IIF
                                      (
                                        [TO].[Code] = 'PO' OR ([TT].[Code] IN('RAPA', 'PAFA', 'BAPR', 'COEX') AND [S].[Code] = 'InProgress'),
                                        NULL,
          ...
          AND ( [TRD].[InternalDescription] = @merchantId OR @merchantId IS NULL)
                    ORDER BY
            CASE WHEN [T].[ProcessedDate] IS NULL THEN 1 ELSE 0 END,
            ISNULL(CAST([T].[ProcessedDate] AS DATE), CAST([T].[TransactionDate] AS DATE)) ASC,
            1 ASC,
            ISNULL(CAST([T].[ProcessedDate] AS TIME), CAST([T].[TransactionDate] AS TIME)) ASC

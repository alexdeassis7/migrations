CREATE OR ALTER PROCEDURE [LP_Operation].[ChangeStatusFromOnHoldToReceived]
                                      (
                                        @Ticket			VARCHAR(15),
...
BEGIN
  ...
  BEGIN TRY
    ...
    DECLARE
        @StatusReceived           [LP_Common].[LP_I_UNIQUE_IDENTIFIER_INT]
	  ...
	  SET @StatusReceived = (SELECT [idStatus] FROM [LP_Common].[Status] WHERE [Code] = 'Received' AND [Active] = 1)
        ...
        WHERE
			[T].[Ticket] = @Ticket
			AND T2.idStatus = @idStatusOnHold

...
GO
/****** Object:  StoredProcedure [LP_Operation].[ListPayOuts]    Script Date: 20/07/2021 13:01:03 ******/
SET ANSI_NULLS ON
GO
...
AS
	BEGIN
		...
		DECLARE @ProviderCode [LP_Common].[LP_F_CODE]
		SET @ProviderCode = CAST(JSON_VALUE(@JSON, '$.provider') AS VARCHAR) 
		...
		-- When country is ARG then @walletBankCode value will be 0000 to discard these transactions in the results 
		DECLARE @WalletBankCode VARCHAR(5) 
		SET @WalletBankCode = NULL
		IF @country_code = 'ARG' AND @ProviderCode NOT IN ('ITAU')
		BEGIN
			SET @WalletBankCode = '00000'
		END
		...
		SELECT [TL].[LotNumber]
				...
				WHERE
					...
					AND ( [BC].[Code] NOT IN (@WalletBankCode) OR @WalletBankCode IS NULL )
					AND [TD].[NetAmount] > 0

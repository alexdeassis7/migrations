/****** Object:  StoredProcedure [LP_Operation].[COL_Payout_BANCOLOMBIA_Bank_Operation_Upload]    Script Date: 9/15/2020 8:53:00 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [LP_Operation].[COL_Payout_BANCOLOMBIA_Bank_Operation_Upload]
                                      (
										@JSON					NVARCHAR(MAX)
...
BEGIN
  BEGIN TRY
    DECLARE
      ...
      SET @idProvider = ( SELECT [idProvider] FROM [LP_Configuration].[Provider] WHERE [Code] = 'BCOLOMBIA' AND [idCountry] = @idCountry AND [Active] = 1 )
	...
	BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          UPDATE  [LP_Operation].[TransactionInternalStatus]
          SET   
            [idInternalStatus] = [LP_Operation].[fnGetIdInternalStatusByCodeCatalogCountryProvider](49, 9, @DetailStatus, 'SCM')
          WHERE 
            [idTransaction] = @idTransaction

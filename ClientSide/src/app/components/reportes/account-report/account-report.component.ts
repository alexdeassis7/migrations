...
export class AccountReportComponent implements OnInit {
  ...
  itemsBreadcrumb: any = ['Home', 'Reports', 'Account Report'];
  ...
  ngOnInit() {
    ...
    this.clientData.setCurrentView(EnumViews.REPORTS, 'AccountReport');
    ...
    this.listColumns = [/*Datos*/'Processed Date', /*'Payment Date',*/  'Ticket', /*'Provider',*/ 'Type Of TX', 'Automatic', 'ID Lot', 'Merchant', 'SubMerchant','Merchant ID',  'Status', 'Detail Status',
  ...
  filterReport() {
    ...
    this.paramsFilter = {
      "idEntityAccount": (this.userPermission == 'ADMIN') ? this.merchantSelect.idEntityUser : this.securityService.currentUser.idEntityUser,
      "cycle": null,
    ...
    let countryCode = (this.userPermission == 'ADMIN') ? this.merchantSelect.CountryCode : this.securityService.currentUser.Country.Code;
    
    this.listColumns[indexColumnTax] = countryCode == "COL" ? "GMF 0.4%" : countryCode == "ARG" ? "Local Tax"  : countryCode == "BRA" ? "IOF 0.38%" : "";
  ...
  exportAgrouped(){
    var dateFrom = this.paramsFilter.dateFrom == null ? this.dateFrom.toISOString().replace(/-|T.*/g, '') : this.paramsFilter.dateFrom.toISOString().replace(/-|T.*/g, '');
    var dateTo = this.paramsFilter.dateTo == null ? this.dateTo.toISOString().replace(/-|T.*/g, '') : this.paramsFilter.dateTo.toISOString().replace(/-|T.*/g, '');
    ...
    let body = {
      FileName: 'Account_Report_Agrouped_' + dateFrom + '_' + dateTo,
      Name: "LP_Filter.GetAccountReportAgrouped",
      IsSP:true,
  ...
  exportToExcel() {
    ...
    this.LpServices.Export.testExport(body).subscribe((data: any) => {
      if (data != null) {
        ...
        let nameFile = 'AccountReport.xlsx';
  ...
  getListReportMerchant(params: any, autoScroll: boolean = false) {
    ...
    this.LpServices.Reports.getListAccountTransactions(params).subscribe((data: any) => {

...
export class MerchantAccountBalanceReportComponent implements OnInit {
  ...
  itemsBreadcrumb: any = ['Home', 'Reports','Merchant' ,'Merchant Balance Report'];
  ...
  ngOnInit() {
    ...
    this.clientData.setCurrentView(EnumViews.NEWREPORTS, 'MerchantAccountBalanceReport');
  ...
  getListClients() {
    let customerId = (this.userPermission == "COMMON") ? this.securityService.UserLogued.idEntityAccount : '0';
    this.LpServices.Filters.getListClients(customerId.toString()).subscribe((data: any) => {
      if (data != null) {
  ...
  getListReportMerchant(params: any, autoScroll: boolean = false) {
    this.LpServices.Reports.getListMerchantBalanceReport(params).subscribe((data: any) => {
      if (autoScroll == false) {

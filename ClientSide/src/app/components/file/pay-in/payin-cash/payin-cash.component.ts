...
@Component({
  selector: 'app-payin-cash',
  templateUrl: './payin-cash.component.html',
  styleUrls: ['./payin-cash.component.css']
})
export class PayinCashComponent implements OnInit {

  filesUpload: any[] = [];
  fileBase64: any;
  res: any = [];
  action: string;
  itemsBreadcrumb: any = ['Home', 'Files Manager', 'PayIn', 'Cashpayment'];
  ...
  constructor(private http: HttpClient, private clientData: ClientDataShared, private modalService: BsModalService, private modalServiceLp: ModalServiceLp,
    private securityService: LpSecurityDataService, private LpServices: LpMasterDataService) { }

  ngOnInit() {

    this.clientData.refreshItemsBreadCrumb(this.itemsBreadcrumb);
    this.clientData.setCurrentView(EnumViews.FILE_MANAGER, 'Payin','PayinCashPayment');
    this.action = 'upload';
  ...
  uploadServer() {
    ...
    this.LpServices.Payin.postFilePayIn(params).subscribe((data: any) => {
      ...
      this.modalServiceLp.openModal('ERROR', 'API ERROR',       error.error);

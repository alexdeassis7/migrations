...
@Component({
  selector: 'app-payout-inprogress-to-recieved',
  templateUrl: './payout-inprogress-to-recieved.component.html',
  styleUrls: ['./payout-inprogress-to-recieved.component.css']
})
export class PayoutInprogressToRecievedComponent implements OnInit {

  itemsBreadcrumb: any = ['Home', 'Files Manager', 'PayOut', 'Edit Manual TXS InProgress to Recieved'];
  filteredPayouts: any[] = [];
  ListProviders: any[] = [];
  ...
  ngOnInit() {
    ...
    this.clientData.setCurrentView(EnumViews.FILE_MANAGER, 'PayOut', 'PayOutManualRecieved');
    this.getProviders();
    this.getCountries();
    this.getListClients();
  ...
  changeStatus(update: string)
  {
    ...
    (<ModalConfirmComponent>this.modalRef.content).confirmAction.subscribe(response => {
      if (response == true) {
        this.modalServiceLp.showSpinner();
        this.LpServices.Payout.revertDownload(this.checkedPayouts,this.countryCode).subscribe((data : any) =>{

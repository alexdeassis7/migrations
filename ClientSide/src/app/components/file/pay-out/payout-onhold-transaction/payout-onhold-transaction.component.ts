...
@Component({
  selector: 'app-payout-onhold-transaction',
  templateUrl: './payout-onhold-transaction.component.html',
  styleUrls: ['./payout-onhold-transaction.component.css']
})
export class PayoutTransactionManualOnHoldComponent implements OnInit {

  itemsBreadcrumb: any = ['Home', 'Files Manager', 'PayOut', 'Manual Cancel/Received TXS OnHold'];
  filteredPayouts: any[] = [];
  ...
  ListStatus: any = [{name: 'Cancel', val: false},{name: 'Received', val: true}]
  ListMerchants: any[] = [];
  ...
  onHoldAll: boolean = false;
  modalRef: BsModalRef;
  rejectMessage: string = "Do you want to cancel selected transactions?"
  onHoldMessage: string = "Do you want to move to Received selected transactions?"
  ...
  ngOnInit() {
    ...
    this.clientData.setCurrentView(EnumViews.FILE_MANAGER, 'PayOut', 'PayOutManualOnHold');
    this.getCountries();
    this.getListClients();
  ...
  filter(){
    ...
    this.LpServices.Payout.payoutsFromOnHoldToReceivedOrCancel(params,this.countryCode).subscribe((data : any) =>{
  ...
  changeStatus(reject: string)
  {
    ...
    (<ModalConfirmComponent>this.modalRef.content).confirmAction.subscribe(response => {
      if (response == true) {
        this.modalServiceLp.showSpinner();
        this.LpServices.Payout.manageCancelReceivedPayouts(this.checkedPayouts,this.countryCode,this.statusCode).subscribe((data : any) =>{
